...
//! Synthesizing that keypress is easy. Landing it is the hard part, and there are exactly two ways,
//! which is why the user picks between them:
//!
//! - **Deferred.** Leave this file alone and wait for the game window, where 3DMigoto's shipped
//!   `check_foreground_window = 1` already accepts hotkeys. Costs nothing here, but the keypress now
//!   has to cross into an elevated process, so Eous must run elevated too — see `commands::reload`.
//! - **Immediate.** Set the key to `0` so 3DMigoto acts on the F10 while *our* window is in front.
//!   Needs no elevation, because the injected input never leaves a window we own. The cost is that
//!   the setting is not scoped to our keypress: 3DMigoto now acts on every hotkey system-wide, and
//!   mods routinely bind bare letters and digits, so typing elsewhere toggles them.
//!
//! Only the second touches this file. Because it is a file we don't own, the edit is opt-in, it
//! records the value it replaced, and it only ever reverts a line it can prove it wrote — see
//! [`MARKER`].
