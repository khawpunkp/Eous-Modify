...
<template>
   <!-- .self so only the backdrop itself closes this: a click that started inside the card bubbles
        up here too, and without it, selecting a keybind's text would dismiss the popup. Safe here
        because this popup is read-only — the modals holding unsaved form state deliberately don't
        close this way. -->
   <div
      ...
      >
         ...
         <div class="flex flex-col gap-2" v-auto-animate>
            ...
            <template v-else>
               ...
               <ul v-else v-auto-animate class="flex flex-col gap-2">
                  <li
